...
public partial class SortingControl : UserControl
{
    ...
    {
        if (await GlobalSettings.SafeGetSettingAsync<string>(SettingKeys.SORTING_MODE) ==
            SortingModesComboBox.Text) return;
    ...
    {
        if (await GlobalSettings.SafeGetSettingAsync<string>(SettingKeys.SORTING_ORDER) ==
            SortingOrderComboBox.Text) return;
